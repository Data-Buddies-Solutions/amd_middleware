...
func TestRoutingForCarrierID(t *testing.T) {
	tests := []struct {
		...
		{"PRE04 restricted to enabled medical providers", "car40916", RoutingBachOnly, false},
...
func TestLookupInsurance_HollywoodSweetwaterMedicalABachOverrides(t *testing.T) {
	...
	tests := []struct {
		...
		{"hollywood accepts humana medicaid hmo with preauth", hollywood, "Humana Medicaid HMO", "", true},
		...
		{"hollywood accepts united individual exchange network with preauth", hollywood, "United Healthcare Individual Exchange Network (Medical)", "", true},
